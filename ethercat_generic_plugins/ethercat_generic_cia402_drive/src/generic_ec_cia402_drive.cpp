    ...
    if (pdo_channels_info_[index].index == CiA402D_RPDO_CONTROLWORD) {
      if (is_operational_) {
        ...
        if (mode_of_operation_ == ModeOfOperation::MODE_HOMING) {
          ...
          std::cerr
              << "EcCiA402Drive: Setting last_position_ tot NAN for DRIVE "
              << for_name_ << std::endl;
          for (auto &channel : pdo_channels_info_) {
            if (channel.index == CiA402D_RPDO_POSITION) {
              ...
              std::cerr
                  << "EcCiA402Drive: Setting last value tot NAN for DRIVE "
                  << for_name_ << std::endl;
    ...
    if (index == all_channels_.size() - 1) { // if last entry  in domain
      if (status_word_ != last_status_word_) {
        ...
        if (state_ != last_state_) {
          std::cout << "STATE[" << alias << "]: " << DEVICE_STATE_STR.at(state_)
  ...
  bool EcCiA402Drive::setupSlave(
    ...
    if (paramters_.find("command_interface/position") != paramters_.end()) {
      position_command_interface_index_ =
          std::stoi(paramters_["command_interface/position"]);
    }
