...
namespace ethercat_controllers {
  ...
  CallbackReturn CiA402Controller::on_configure(const rclcpp_lifecycle::State
    ...
    control_words_.resize(
        dof_names_.size(), std::numeric_limits<double>::quiet_NaN()
    );
    ...
    try {
      ...
      rt_drive_state_publisher_ =
          std::make_unique<DriveStatePublisher>(drive_state_publisher_);
    } catch (const std::exception &e) {
      RCLCPP_ERROR(
          get_node()->get_logger(),
    ...
    for (auto i = 0ul; i < dof_names_.size(); i++) {
      ...
      if (start_homing_request && (*start_homing_request)) {
        ...
        control_word = control_word |
            0b00010000 command_interfaces_[3 * i].set_value(control_word
            ); // control_word
        ...
        uint16_t control_word = command_interfaces_[3 * i].get_value();
        uint16_t control_word = control_word & 0b11101111;
